...
?>
	<footer id="colophon" class="site-footer" role="contentinfo">
		<div class="site-info">
			...
			<?php 
			$statistics_link = akina_option('site_statistics_link') ? '<a href="'.akina_option('site_statistics_link').'" target="_blank">Statistics</a>' : '';
			$site_map_link = akina_option('site_map_link') ? '<a href="'.akina_option('site_map_link').'" target="_blank">Sitemap</a>' : '';
			printf( esc_html__( '%1$s &nbsp; %2$s &nbsp; %3$s &nbsp; %4$s', 'akina' ), $site_map_link, '<a href="http://www.akina.pw/themeakina" rel="designer" target="_blank">Theme</a>', '<a href="https://wordpress.org/" target="_blank">WordPress</a>', $statistics_link); 
			?>
			</div>
		</div><!-- .site-info -->
	</footer><!-- #colophon -->
	...
	<!-- search end -->
	<?php if ( akina_option('preloader') == 'yes' && !akina_option('poi_pjax')) { //全局加载动画?>
	<div id="loading">
		<div id="loading-center">
		  	<div class="loader">
	            <div class="loading-one"></div>
	            <div class="loading-two">Loading ...</div>
	        </div>
        </div>
	</div>
	<?php }else{ //顶部加载条?>
